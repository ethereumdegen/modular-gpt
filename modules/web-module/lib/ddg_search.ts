/*
Taken from chatgpt-advanced 
*/

import axios from "axios";

import cheerio from "cheerio";
import { getWebpageTitleAndText } from "./api";
...
export async function getHtml({
  ...
  try {
    const response = await axios(`${BASE_URL}/lite/`, {
      method: "POST",
      headers,
    ...
    throw new Error(
      `Failed to fetch: ${error.response.status} ${error.response.statusText}`
    );
  }
}
...
function htmlToSearchResults(html: string, numResults: number): SearchResult[] {
  ...
  const webLinks = $(`table:nth-of-type(${numTables}) .result-link`).slice(
    0,
    upperBound
  );
  const webSnippets = $(
    `table:nth-of-type(${numTables}) .result-snippet`
  ).slice(0, upperBound);
...
export async function webSearch(
  ...
  const response: SearchResponse = await getHtml(search); //Browser.runtime.sendMessage
